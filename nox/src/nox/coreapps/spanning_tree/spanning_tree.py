...
class Spanning_Tree(Component):
    ...
    def update_spanning_tree_callback(self, links):
        ...
        As a temporary hack to deal with the fact that we do not have
        ...
        which represent links that do not contain other OpenFlow
        switches. This set of paths can be used safely for flooding to
        ensure that we do not circulate broadcast packets.
        ...
        for dp in self.datapaths.iterkeys():
            ...
            logger.debug("Ports for %s: Flood: %s   Non-flood: %s"%(longlong_to_octstr(dp)[6:], floodports, nonfloodports))
    ...
    def handle_packet_in(self, dpid, inport, reason, len, bufid, packet):
        ...
        if not packet.parsed:
            logger.warn('Ignoring incomplete packet',system='spanning_tree')
        ...
        try:
            ...
            logger.debug("Error checking destination")
            print e
            pass
        ...
        try:
            if self.datapaths[dpid][inport]['flood']:
                ...
                logger.debug("Port Comes in on a disabled port. But we don't care.")
                return CONTINUE
        except KeyError, e:
            logger.debug("Some key not found")
            print e
            return STOP
