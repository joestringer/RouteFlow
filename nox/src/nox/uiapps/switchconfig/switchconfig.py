...
from nox.netapps.directory.pydirmanager import Principal_rename_event, Principal_delete_event
...
class switchconfig(Component):
    ...
    def handle_principal_rename_event(self,event):
    ...
    def handle_principal_delete_event(self,event):

      if event.type != Directory.LOCATION_PRINCIPAL:
        return CONTINUE

      oldname = self.nm.get_name(event.id)
      if oldname in self.nat_config:
        to_change = { oldname : [] }
        self.simple_config.set_config("switch_nat_config",to_change)
      return CONTINUE
    ...
    def install(self):
        ...
        self.register_handler(Principal_rename_event.static_get_name(),
                              self.handle_principal_rename_event)
        self.register_handler(Principal_delete_event.static_get_name(),
                              self.handle_principal_delete_event)
