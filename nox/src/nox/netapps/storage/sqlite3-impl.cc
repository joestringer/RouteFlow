...
SQLite_storage::configure(const container::Configuration* conf) {
    string database;
    if (DATED_SQLITE_FILENAME)
    {
      time_t datetime;
      time(&datetime);
      database = ctime(&datetime);
      std::replace(database.begin(), database.end(), ' ','-');
      database = database.substr(0, database.length()-1);
      database += ".sqlite";
    }
    else
      database = DEFAULT_SQLITE_FILENAME;
