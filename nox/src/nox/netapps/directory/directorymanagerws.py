...
from nox.netapps.authenticator.pyauth import Host_event, User_event
from nox.netapps.authenticator.pyauth import Host_auth_event, User_auth_event
from nox.netapps.authenticator.pynamemanager import NameManager, PyNameManager
...
from nox.uiapps.coreui.webservice import NOT_DONE_YET, WSPathArbitraryString 
from nox.uiapps.coreui.web_arg_utils import *
from nox.netapps.directory.query import query
...
class directorymanagerws(Component):
    ...
    def _do_delete_bindings(self, request, arg):
        ...
        try:
            ...
            ae = None
            if type_str == "user":
                id = self.nm.get_principal_id(principalid,
                                              Directory.USER_PRINCIPAL,
                                              False, False)
                if id != NameManager.UNCREATED_ID:
                    ae = User_auth_event(id,
                                         NameManager.UNKNOWN_ID,
                                         User_event.DEAUTHENTICATION_EVENT)
            elif type_str == "host":
                id = self.nm.get_principal_id(principal_id,
                                              Directory.HOST_PRINCIPAL,
                                              False, False)
                if id != NameManager.UNCREATED_ID:
                    ae = Host_auth_event(datapathid.from_host(0), 0,
                                         create_eaddr(0), 0, id,
                                         Host_auth_event.EF_HOSTNAME,
