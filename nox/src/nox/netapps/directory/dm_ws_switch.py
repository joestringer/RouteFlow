...
from nox.uiapps.coreui.webservice import *
from nox.netapps.directory.query import query
from nox.netapps.directory.directorymanagerws import *
from nox.netapps.directory.directorymanager import mangle_name
        ...
        try:
            ...
            if dpid in self.switchstats.dp_port_stats:
                ...
                for i in range(len(ports)):
                    if ports[i]['port_no'] == portno:
                        del ports[i]['port_no']
                        request.write(simplejson.dumps(ports[i]))
                        ports[i]['port_no'] = portno
                        request.finish()
                        return NOT_DONE_YET
