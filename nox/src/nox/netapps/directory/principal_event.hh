...
struct Principal_rename_event
    : public Event,
      boost::noncopyable
{
    Principal_rename_event(applications::directory::Principal_Type type_,
                           uint32_t id_, const std::string& oldname_,
                           const std::string& newname_);

    // -- only for use within python
    Principal_rename_event() : Event(static_get_name()) { }

    static const Event_name static_get_name() {
        return "Principal_rename_event";
    }
...
struct Principal_delete_event
    : public Event,
      boost::noncopyable
{
    Principal_delete_event(applications::directory::Principal_Type type_,
                           uint32_t id_);
