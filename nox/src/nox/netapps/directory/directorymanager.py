...
from nox.netapps.directory import pydirmanager
from nox.netapps.directory.pynetinfo_mod_event import NetInfo_mod_event
from nox.netapps.directory.pydirmanager import Principal_rename_event
from nox.netapps.directory.pydirmanager import Principal_delete_event
from nox.netapps.directory.pydirmanager import Group_rename_event
from nox.netapps.directory.pydirmanager import Group_delete_event
from nox.netapps.directory.pydirmanager import Group_change_event
...
class directorymanager(Component):
    ...
    def __init__(self, ctxt):
        ...
        from nox.netapps.directory.discovered_directory import discovered_directory
        ...
        self.register_handler(Host_bind_event.static_get_name(),self.host_event)
        ...
        self.register_handler(Principal_rename_event.static_get_name(),
                self.renamed_principal)
        self.register_handler(Group_rename_event.static_get_name(),
                self.renamed_group)
        self.register_handler(Principal_delete_event.static_get_name(),
                self.deleted_principal)
        self.register_handler(Group_delete_event.static_get_name(),
                self.deleted_group)
    ...
    def _delegate_mod_group_membership(self, group_type, group_name,
        ...
        def _post_modify_event(res):
            ...
            m_group_id = self.nm.get_group_id(m_group_name.encode('utf-8'),
                                              group_type, False, True)
            if add_members:
            ...
            for m in member_names:
                if isinstance(m, basestring):
                    ...
                    memberstr = str(m)
                if group_type == Directory.DLADDR_GROUP \
                        or group_type == Directory.NWADDR_GROUP:
                    e = Group_change_event(group_type, m_group_id,
                                           m_change_type, memberstr)
                else:
                    member_id = self.nm.get_principal_id(memberstr,
                                                         Directory.PRINCIPAL_GROUP_TO_PRINCIPAL[group_type],
                                                         False, True)
                    e = Group_change_event(group_type, m_group_id,
                                           m_change_type, member_id)
                self.post(e)
            for sg in subgroup_names:
                sg_id = self.nm.get_group_id(sg.encode('utf-8'), group_type, False, True)
                e = Group_change_event(group_type, m_group_id,
                                       sg_change_type, sg_id)
                self.post(e)
            return res
    ...
    def rename_principal(self, principal_type, old_name, new_name,
        ...
        def _post_ren_event(res):
            ...
            p_id = self.nm.get_principal_id(oldname, principal_type, False, True)
            self.nm.rename_principal(oldname, newname, principal_type)
            pne = Principal_rename_event(principal_type, p_id, oldname, newname)
            self.post(pne)
            ...
            if principal_type == Directory.SWITCH_PRINCIPAL and \
                ...
                for loc in res.locations:
                    ...
                    if newlname != oldlname:
                        ...
                        p_id = self.nm.get_principal_id(oldlname, lptype, False, True)
                        self.nm.rename_principal(oldlname, newlname, lptype)
                        pne = Principal_rename_event(lptype, p_id, oldlname, newlname)
                        self.post(pne)
            return res
    ...
    def del_principal(self, principal_type, name, dir_name="",
        ...
        def _add_default_loc(res, state='entry', old_loc=None):
            ...
            if state == 'entry':
                ...
                ### no longer post location_delete_event
        ...
        def _post_del_event(pi):
            if pi is not None:
                ...
                oldname = mangle_name(dm_dir, dm_name).encode('utf-8')
                old_id = self.nm.get_principal_id(oldname, principal_type, False, True)
                pde = Principal_delete_event(principal_type, old_id)
                self.nm.delete_id(old_id)
                self.post(pde)
                if principal_type == Directory.SWITCH_PRINCIPAL:
                    if hasattr(pi, 'locations'):
                        for loc in pi.locations:
                            ...
                            oldlname = oldlname.encode('utf-8')
                            lptype = Directory.LOCATION_PRINCIPAL
                            old_lid = self.nm.get_principal_id(oldlname, lptype, False, True)
                            pde = Principal_delete_event(lptype, old_lid)
                            self.post(pde)
                            self.nm.delete_id(old_lid)
    ...
    def rename_group(self, group_type, old_name, new_name, old_dir_name,
        ...
        def _post_ren_event(res):
            gid = self.nm.get_group_id(oldname, group_type, False, True)
            self.nm.rename_group(oldname, newname, group_type)
            gne = Group_rename_event(group_type, gid, oldname, newname)
            self.post(gne)
            return res
    ...
    def del_group(self, group_type, group_name, dir_name="", post_event=True):
        def _post_del_event(gi):
            if gi is not None:
                gid = self.nm.get_group_id(gi.name.encode('utf-8'), group_type, False, True)
                gde = Group_delete_event(group_type, gid)
                self.post(gde)
                self.nm.delete_id(gid)
            return gi
...
#      return mangle_name(self.discovered_dir.name, host_name)
    ...
    def host_event(self, event):
      hostname = self.nm.get_name(event.hostname)
      dir,pname = demangle_name(hostname) 
      ...
      if(event.action == Host_bind_event.ADD):
        fn = self.host_join
      else: 
        fn = self.host_leave
      ...
      def get_ok(host_info): 
        if host_info: 
          if host_info._refcount > 1:
            ...
            d = self.del_principal(Directory.HOST_PRINCIPAL, name, 'discovered')
          d.addErrback(err) 
    ...
    def dp_leave(self,dpid):
      ...
      def ok(res): 
        for switchname in res: 
          self.del_principal(Directory.SWITCH_PRINCIPAL, switchname, 'discovered')
          # ignore deferred
    ...
    def deleted_principal(self, event):
        name = self.nm.get_name(event.id)
        if demangle_name(name)[0] == self.discovered_dir.name:
            # directorymanager has already added the new name to global groups
            return CONTINUE
        self.discovered_dir.rename_group_member(None, event.type, name, '')
        return CONTINUE

    def deleted_group(self, event):
        name = self.nm.get_name(event.id)
        if demangle_name(name)[0] == self.discovered_dir.name:
            # directorymanager has already added the new name to global groups
            return CONTINUE
        self.discovered_dir.rename_group_subgroup(None, event.type, name, '')
        return CONTINUE
